...
const unsigned long TAP_TIMEOUT = 200;

// BOOT/user button GPIOs are loaded from the detected LilyGo board config at runtime.
const int CONFIG_BUTTON_PIN = 0;
const unsigned long WIFI_RESET_HOLD_MS = 10000;
...
void openWifiResetPortal(int sourcePin);
...
void setup() {
    ...
    Serial.printf("Config GPIO%d initial state: %d (LOW = pressed)\n",
                  CONFIG_BUTTON_PIN, digitalRead(CONFIG_BUTTON_PIN));
...
void loop() {
    ...
    // Config button (hold 10 seconds to wipe Wi-Fi and reopen setup portal)
    static unsigned long buttonPressTime = 0;
    static bool configHandledForCurrentPress = false;
    static unsigned long lastPrintedSecond = (unsigned long)-1;
    const unsigned long now = millis();

    if (isConfigButtonPressed()) {
        if (buttonPressTime == 0) {
            buttonPressTime = now;
            lastPrintedSecond = (unsigned long)-1;
            Serial.printf("Config button press detected on GPIO%d\n", CONFIG_BUTTON_PIN);
        }

        unsigned long heldMs = now - buttonPressTime;
        unsigned long heldSec = heldMs / 1000;

        if (heldSec != lastPrintedSecond && heldSec < 10) {
            lastPrintedSecond = heldSec;
            Serial.printf("Config hold progress: %lus / 10s (GPIO%d)\n", heldSec, CONFIG_BUTTON_PIN);
            if (statusLabel) {
                char buf[64];
                snprintf(buf, sizeof(buf), "BOOT hold: %lus / 10s", heldSec);
                lv_label_set_text(statusLabel, buf);
            }
        }

        if (!configHandledForCurrentPress && heldMs >= WIFI_RESET_HOLD_MS) {
            configHandledForCurrentPress = true;
            openWifiResetPortal(CONFIG_BUTTON_PIN);
            buttonPressTime = 0;
        }
    } else {
        if (buttonPressTime != 0) {
            unsigned long heldMs = now - buttonPressTime;
            Serial.printf("Config button released after %lums (GPIO%d)\n", heldMs, CONFIG_BUTTON_PIN);
            updateStatusLabel();
        }
        buttonPressTime = 0;
        configHandledForCurrentPress = false;
        lastPrintedSecond = (unsigned long)-1;
    }
}
...
void openWifiResetPortal(int sourcePin) {
    Serial.printf("Config button held 10s on GPIO%d - wiping WiFi settings and starting portal\n", sourcePin);
    if (statusLabel) lv_label_set_text(statusLabel, "Wiping WiFi... starting setup");

    // Clear Wi-Fi config from WiFiManager and from ESP32 Wi-Fi stack/NVS defaults.
...
void startWebServer() {
    ...
    server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        html += "<p>Hold BOOT button 10s to reconfigure WiFi</p>";
